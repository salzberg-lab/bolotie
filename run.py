...
def load_paths(path_fname,seqs_fname):
    ...
    with open(path_fname,"r") as inFP:
        for line in inFP.readlines():
            ...
            assert name not in paths,"duplicate names"
            paths[name] = [signature]
    ...
    with open(seqs_fname,"r") as inFP:
        ...
        for line in inFP.readlines():
            if line[0]==">":
                if write_seq:
                    paths[cur_name].append(cur_seq)
                cur_name = line[1:].strip()
                cur_seq = ""
                if cur_name in paths:
                    write_seq = True
                else:
                    write_seq = False
            else:
...
def get_stages(args):
    ...
    for stage in all_stages:
        if args.clusters is None and stage=="index":
            sys.stderr.write("can not perform indexing, path finding, parents finding and plotting without provided clusters. You can use consensus calls to generate a tree and clusters\n")
            return run_stages
...
def load_vars(vars_fname,clu_fname):
    ...
    # merge
    prev_len = len(vars_df)
    vars_df = vars_df.merge(clu_df,how="left",on="seqid",indicator=True)
    # assert prev_len==len(vars_df),"number of rows"
    # assert len(vars_df)==len(vars_df[vars_df["_merge"]=="both"]),"not all merged"
    # assert len(set(clu_df["seqid"]))==len(set(vars_df["seqid"])),"not the same seqids"
...
def plot_recomb(axes,seq,mat,counts,totals,clus_plot,clus_scatter,palette,thresh=0.1):
    ...
    for i,nt in enumerate(seq):
        for clu in clus_plot:
            ...
            if abs(prob-base_prob)>=0.09:
                ...
                total_count = int(totals[i][acgt[nt]])
    ...
    for i,rec_clu in enumerate(clus_scatter):
        ...
        axes.scatter(list(probs.index),probs[rec_clu],s=mark_size,marker=markers[i],color=palette.get(rec_clu,'#333333'),label=str(rec_clu))
        ...
        if rec_clu in clus_plot:
            ...
            for i,j in zip(list(res["xs"])[1:],list(res["prob"])[1:]):
...
def load_parents(parents_fname,mat):
    ...
    with open(parents_fname,"r") as inFP:
        for line in inFP.readlines():
            ...
            rec = lineCols[0]
            par1c = int(lineCols[1])
            par1 = lineCols[4]
            par2c = int(lineCols[6])
            par2 = lineCols[9]
            parents[rec] = [0 for x in range(mat.shape[1])]
            parents[rec][par1c] = par1
            parents[rec][par2c] = par2
...
def align(args,bolotie_path,tmp_dir):
    od = args.outdir.rstrip("/")+"/"
...
def consensus(args,bolotie_path,tmp_dir):
    od = args.outdir.rstrip("/")+"/"
...
def index(args,bolotie_path,tmp_dir):
    od = args.outdir.rstrip("/")+"/"
...
def paths(args,bolotie_path,tmp_dir):
    od = args.outdir.rstrip("/")+"/"
...
def parents(args,bolotie_path,tmp_dir):
    od = args.outdir.rstrip("/")+"/"
...
def plot(args,figure_dir):
    ...
    for _,c in cold.items():
        light_shades.append(shadeColor(c,40))
    ...
    for refname in paths:
        ...
        fig, axes = plt.subplots(nrows=4,ncols=1,figsize=(40,8),sharex=True,gridspec_kw={'hspace':0,'height_ratios':[6,1,1,1]})
        ...
        plot_recomb(axes[0],seq,mat,counts,totals,rec_clus,list(range(mat.shape[1])),cold_dark,thresh=0.1)

        # plot recomb variants
        rec_vars = vars_df[vars_df["seqid"]==refname].sort_values(by="pos")
        for nt in "ACGT":
            xs = rec_vars[rec_vars["query"]==nt]["pos"].to_list()
            heights = [1 for x in range(len(xs))]
...
def wrapper(args):
    ...
    if not os.path.exists(bolotie_path):
        ...
        for tmp_path in glob.glob(cur_path+"/*/*"):
            ...
            if tmp_fname=="bolotie":
                ...
                break
        if not found:
            sys.stderr.write("Did not find bolotie executable path. Please make sure bolotie executable is located within the same directory as the run.py script or whithin a single directory level down.")
            sys.exit(2)
    ...
    if "align" in run_stages:
        align(args,bolotie_path,tmp_dir)
    if "consensus" in run_stages:
        consensus(args,bolotie_path,tmp_dir)
    if "index" in run_stages:
        index(args,bolotie_path,tmp_dir)
    if "path" in run_stages:
        paths(args,bolotie_path,tmp_dir)
    if "parents" in run_stages:
        parents(args,bolotie_path,tmp_dir)
